...
#include <omp.h>
#include "cuda_half.h"
#include "half_kernels/kernels.h"
    ...
    void cuda_evaluate(float* input_pos, float* input_vel, float* input_mass, int n_particles, int steps, float G, float eps, float dt, int n_params, int solver, int v, double *saveTime, double *totalTime, double *copyTime){
        ...
        *copyTime += second-first;
        ...
        switch (solver){
            ...
            case 0:
                force_solve_cheap_sqrt<<<numBlocks,blockSize>>>(d_pos,d_mass,d_acc_phi,G,eps,n_particles);
                break;
            case 1:
                force_solve_cheap_dist<<<numBlocks,blockSize>>>(d_pos,d_mass,d_acc_phi,G,eps,n_particles);
                break;
