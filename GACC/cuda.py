...
f2_lib = package_directory + "/cuda_eval_f2.dll"
cuda_eval_f2 = ctypes.CDLL(f2_lib)

cheap_lib = package_directory + "/cuda_eval_cheap.dll"
cuda_eval_cheap = ctypes.CDLL(cheap_lib)
...
def evaluate(df,steps=0,G=1,eps=0,dt=1/64,n_params=10, solver = 0,outname="out.dat",v=False,precision="f4"):
    ...
    if precision == "f4":
        ...
        cuda_eval_f4.cuda_evaluate(posPtr,velPtr,massPtr,ctypes.c_int(n_particles),ctypes.c_int(steps),ctypes.c_float(G),ctypes.c_float(eps**2),ctypes.c_float(dt),ctypes.c_int(n_params), ctypes.c_int(solver), ctypes.c_int(v), saveTimePtr, totalTimePtr, copyTimePtr)
        ...
        cuda_eval_f8.cuda_evaluate(posPtr,velPtr,massPtr,ctypes.c_int(n_particles),ctypes.c_int(steps),ctypes.c_double(G),ctypes.c_double(eps**2),ctypes.c_double(dt),ctypes.c_int(n_params), ctypes.c_int(solver), ctypes.c_int(v), saveTimePtr, totalTimePtr, copyTimePtr)
    ...
    return out_df[(out_df["id"] < true_n)].reset_index().loc[:,["step","id","x","y","z","vx","vy","vz","ax","ay","az","gpe"]],stats

def half_evaluate(df,steps=0,G=1,eps=0,dt=1/64,n_params=10, solver = 0,outname="out.dat",v=False):
    ...
    cuda_eval_f2.cuda_evaluate(posPtr,velPtr,massPtr,ctypes.c_int(n_particles),ctypes.c_int(steps),ctypes.c_float(G),ctypes.c_float(eps**2),ctypes.c_float(dt),ctypes.c_int(n_params), ctypes.c_int(solver), ctypes.c_int(v), saveTimePtr, totalTimePtr, copyTimePtr)
...
def cheap_evaluate(df,steps=0,G=1,eps=0,dt=1/64,n_params=10, solver = 0,outname="out.dat",v=False):
    ...
    cuda_eval_cheap.cuda_evaluate(posPtr,velPtr,massPtr,ctypes.c_int(n_particles),ctypes.c_int(steps),ctypes.c_float(G),ctypes.c_float(eps**2),ctypes.c_float(dt),ctypes.c_int(n_params), ctypes.c_int(solver), ctypes.c_int(v), saveTimePtr, totalTimePtr, copyTimePtr)
